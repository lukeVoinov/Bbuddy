...
def find_matching_transcript(audio_folder, mp3_name):
    """Find matching transcript file by trying different naming patterns."""
    transcript_folder = os.path.join(audio_folder, "transcriptionSet")
    ...
    for transcript in transcript_files:
        ...
        if "rita" in mp3_name.lower() and "rita" in transcript.lower():
            return os.path.join(transcript_folder, transcript)
        if "james" in mp3_name.lower() and "james" in transcript.lower():
            return os.path.join(transcript_folder, transcript)
    
    return None

def process_audio_folder(audio_folder, model_path, output_folder):
    """Process all MP3 files in a folder and their corresponding transcriptions."""
    try:
        ...
        for audio_path in mp3_files:
            try:
                ...
                mp3_basename = os.path.basename(audio_path)
                mp3_name = os.path.splitext(mp3_basename)[0]
                
                # Find matching transcript
                transcript_path = find_matching_transcript(audio_folder, mp3_name)
                
                # Skip if transcript doesn't exist
                if not transcript_path:
                    print(f"Warning: No matching transcript found for {mp3_basename}")
                    continue
                
                print(f"\nProcessing: {mp3_basename}")
                print(f"Using transcript: {os.path.basename(transcript_path)}")
                
                # Process the file pair
                results = process_complete_transcript(audio_path, transcript_path, model_path)
                
                # Create output filename
                output_path = os.path.join(output_folder, f"results_{mp3_name}.csv")
                
                # Save results
                save_results_to_csv(results, output_path)
                
                # Print summary for this file
                print(f"\nResults for {mp3_basename}:")
                ...
                print(f"Error processing {mp3_basename}: {str(e)}")
                continue
                
    except Exception as e:
        raise RuntimeError(f"Error processing audio folder: {str(e)}")

if __name__ == "__main__":
    # Example usage
    audio_folder = "C:/Users/lukev/Projects/BehavBuddy/mp3TestingSet"
    model_path = "C:/Users/lukev/Projects/BehavBuddy/trained_models/trainedBB_Afive.pkl"
    output_folder = "C:/Users/lukev/Projects/BehavBuddy/mp3TestingSet/testingFFTResults"
    
    try:
        process_audio_folder(audio_folder, model_path, output_folder)
    except Exception as e:
        print(f"Error processing audio folder: {str(e)}")
        import traceback
        print("\nFull error traceback:")
        print(traceback.format_exc())
        
"""
    audio_folder = "C:/Users/lukev/Projects/BehavBuddy/mp3TestingSet"                                  # Folder containing MP3 files
    model_path = "C:/Users/lukev/Projects/BehavBuddy/trained_models/trainedBB_Afive.pkl"               # Path to trained model
    output_folder = "C:/Users/lukev/Projects/BehavBuddy/mp3TestingSet/testingFFTResult"  
"""